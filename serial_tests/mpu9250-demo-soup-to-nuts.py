...
class mpu9250(object):
	...
	def compass_deg(self, m):
		# return (atan2(m[1], m[0]) * (180/pi) + 90 + 360) % 360 # IMU is upside-down?
		return (atan2(m[0], m[1]) * (180/pi) + 90 + 360) % 360 # IMU is upside-down?
...
if __name__ == "__main__":
  ...
  try:
    while True:
      ...
      g = imu.gyro
      m = imu.mag
      print(f'a=[{a[0]: .3f},{a[1]: .3f},{a[2]: .3f}];g=[{g[0]: .3f},{g[1]: .3f},{g[2]: .3f}];m=[{m[0]: .3f},{m[1]: .3f},{m[2]: .3f}]')
      print("Compass (deg)", imu.compass_deg(m) )
